...
sudo k3s kubectl create secret generic nextcloud-certs --from-file=$TMP_CERT_FOLDER -n nextcloud --save-config --dry-run=client -o yaml  | kubectl apply -f -

echo "Updated secret"
sudo k3s kubectl describe secret nextcloud-certs -n nextcloud
