...
sudo k3s kubectl create secret generic registry-certs --from-file=$TMP_CERT_FOLDER -n docker-registry --save-config --dry-run=client -o yaml  | kubectl apply -f -

echo "Updated secret"
sudo k3s kubectl describe secret registry-certs -n docker-registry
